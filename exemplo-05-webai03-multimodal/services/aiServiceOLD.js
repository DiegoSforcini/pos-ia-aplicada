export class AIService {
    ...
    async* createSession(question, temperature, topK, file = null) {
        ...
        try {
            // Try with multimodal capabilities
            ...
            this.isMultimodalSupported = true;
            console.log('Multimodal session created successfully');
        } catch (multimodalError) {
            console.warn('Multimodal creation failed, falling back to text-only:', multimodalError);
            // Fallback to text-only if multimodal isn't supported
            this.session = await LanguageModel.create({
                expectedInputs: [
                    { type: "text", languages: ["en"] },
                ],
        ...
        if (file && this.isMultimodalSupported) {
            ...
            if (fileType === 'image' || fileType === 'audio') {
                try {
                    ...
                    console.log(`✓ Successfully added ${fileType} to prompt:`, {
                        name: file.name,
                        type: file.type,
                        size: file.size,
                        blobSize: blob.size
                    ...
                    console.warn('Proceeding with text-only (file error)');
                }
            } else {
                console.warn('File type not supported:', file.type);
            }
        } else if (file && !this.isMultimodalSupported) {
            console.warn('File provided but multimodal is not supported. Will process text only.', {
                fileName: file.name,
                fileType: file.type
            });
        }

        console.log('📤 Sending prompt to model with content:', {
            textCount: contentArray.filter(c => c.type === 'text').length,
            imageCount: contentArray.filter(c => c.type === 'image').length,
            audioCount: contentArray.filter(c => c.type === 'audio').length,
            isMultimodalSupported: this.isMultimodalSupported
        });
